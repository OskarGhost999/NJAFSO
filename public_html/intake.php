...
if($_POST){
	...
	$city = $_POST["city"];
	$zip = $_POST["zipCode"];
	$county = NULL;           //form does not ask for county

	if(!empty($address1) && !empty($city) && !empty($zip)) {
			
			$result = DBL::save_address($address1, $address2, $city, $zip, $county);

			if(Common::get($result, "status", 400) == 200){
				$address_id = Common::get($result, "data", NULL);
			}
			else { 
				Common::flash("Address submission error", "danger");
				//var_export($result);
				echo "<script> ; window.location.href='intake.php'; </script>";
			}
		}
		else {
			Common::flash("Address fields must not be empty", "warning");
			echo "<script> ; window.location.href='intake.php'; </script>";
		}
	...
	if (!empty($firstname) && !empty($lastname) && !empty($home_phone) && !empty($email)) {
			
			$result = DBL::save_personal_info($firstname, $lastname, $prefix, $middlename, $dob, $gender, $race,
			...
			if(Common::get($result, "status", 400) == 200){
				$person_id = Common::get($result, "data", NULL);
			}
			else { 
				Common::flash("Personal information submission error", "danger");
				echo "<script> ; window.location.href='intake.php'; </script>";
			}
		}
		else {
			Common::flash("Personal information fields must not be empty", "warning");
			echo "<script> ; window.location.href='intake.php'; </script>";
		}

	$programstartdate = $_POST["ProgramStartDate"];
	$casenumber = $_POST["caseNumber"];
	$caremanager = $_POST["careManager"];
	$dyfscontact = $_POST["DYFSContact"];

	if(!empty($programstartdate) && !empty($casenumber) && !empty($caremanager) && !empty($dyfscontact)) {
			
			$result = DBL::save_case($programstartdate, $casenumber, $caremanager, $dyfscontact, $person_id);

			if (Common::get($result, "status", 400) == 200){
				//echo "case submitted succesfully<br>";
			}
			else { 
				Common::flash("Case submission error", "danger");
				echo "<script> ; window.location.href='intake.php'; </script>";
			}
		}
		else {
			Common::flash("Case fields must not be empty", "warning");
			echo "<script> ; window.location.href='intake.php'; </script>";
		}	
	...
	if( $_POST['CyberNumber'] != NULL){
			...
			$result = DBL::save_cmo_record($cybernumber, $childlevelcare, $childplacement, $childDiagnosis, $childenrollmentedate,
                                            $cmodischargedate, $cmostatus, $dcppinvolvement, $courtinvolvement, $person_id);

			if (Common::get($result, "status", 400) == 200){
				//echo "cmo_record submitted succesfully<br>";
			}
			else { 
				Common::flash("CMO Record submission error", "danger");
				echo "<script> ; window.location.href='intake.php'; </script>";
			}
	}
    ...
    echo "<script> ; window.location.href='intake.php'; </script>";
    //die(header("Location: intake.php"));
